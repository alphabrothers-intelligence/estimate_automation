...
def _form_block(form: FormSpec) -> str:
    ...
    if "description" not in (form.labels or {}):
        # 상품구성 칸이 없는 양식(블렌디드랩·썬데이워커)도 description을 받아야 한다 —
        # 발급 시 품명 아래 괄호에 슬래시로 이어 붙여 찍는다(pdf_service._fold_name_only).
        # 예전엔 여기서 "null로 둘 것"이라고 시켜서 괄호에 넣을 내용이 아예 없었다(2026-08-24).
        lines.append(
            "- description: 이 양식에는 상품구성 전용 칸이 없지만, 발급 시 품명 아래 괄호에"
            ' "/"로 이어 붙여 찍으므로 반드시 채울 것'
        )
    return "\n".join(lines)
...
7. 간접비(경비·일반관리비·이윤)
   이 이름의 항목이 카탈로그에 있으면 용역 사업 견적서다. 나머지 항목(직접비)을 먼저 다 산정한 뒤,
   - 경비: 실비다. 방문·회의 인원과 횟수에 맞춰 수량을 잡고 단가는 만원 미만 실비 단위를 그대로 쓴다.
   - 일반관리비: 직접비 합계의 3% 내외(5%를 넘기지 않는다).
   - 이윤: 직접비와 일반관리비 합계의 10% 이내. **이 항목만은 만원 단위로 안 떨어져도 된다** —
     총액을 정확히 맞추려고 역산하는 자리이기 때문이다. 서버가 마지막에 잔액을 여기로 몰아준다.
   간접비를 키워 직접비 단가를 낮추지 않는다. 직접비가 먼저고 간접비는 그 위에 얹는 값이다.
...
2. 다른 표현, 같은 내용
   ...
   - **예외: 구분명 "경비 및 간접비"와 그 안의 항목명("교통비 …", "회의 및 기타경비",
     "일반관리비", "이윤")은 원가계산 표준 명칭이라 한 글자도 바꾸지 않는다.** 다른 업체 견적서도
     똑같이 이 단어를 쓰고, 바꾸면 발주처가 원가 항목을 못 알아본다. 금액만 이 양식 기준으로 다시 잡는다.
...
8. 상세설명 형식
   ...
   - **최대 3줄, 한 줄 25자 이내.** 칸 폭이 좁아 길게 쓰면 글자가 줄줄이 접혀 표가 세로로 늘어난다.
   - [양식 정의]의 description 컬럼명이 "규격/단위"·"규격"처럼 규격 표기 칸이면 1~2줄, 한 줄 12자 이내로 더 줄인다.
   - **간접비 항목(교통비·회의 및 기타경비·일반관리비·이윤)은 번호 없이 한 줄만 쓴다** (25자 이내, 줄바꿈 없음).
     원가계산 표준 항목이라 여러 줄로 풀어쓸 내용이 아니다. 예: "수요처 방문 왕복 교통비 실비".
   - 역슬래시 n을 글자 그대로 쓰지 않는다. JSON 문자열의 줄바꿈 이스케이프만 쓴다.
